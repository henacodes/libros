...
export function parseDownloadUrls(
  ...
  try {
    const mainDownloadUrlElement = document.querySelector(
      Selector.MAIN_DOWNLOAD_URL_SELECTOR
    );
    ...
    const otherDownloadUrlsElements = Array.from(
      otherDownloadUrlsContainerElement.children
    );
